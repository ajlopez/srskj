...
public class Wallet {
    ...
    @GuardedBy("accessLock")
    private final Map<Address, byte[]> accounts = new HashMap<>();

    @GuardedBy("accessLock")
    private final List<Address> initialAccounts = new ArrayList<>();

    private final Object accessLock = new Object();
    private final Map<Address, Long> unlocksTimeouts = new HashMap<>();
    ...
    public List<byte[]> getAccountAddresses() {
        ...
        Set<Address> keys = new HashSet<>();

        synchronized(accessLock) {
            for (Address addr: this.initialAccounts) {
                addresses.add(addr.getBytes());
            }

            for (byte[] address: keyDS.keys()) {
                keys.add(new Address(address));
            }
    ...
    public Address addAccount() {
        Account account = new Account(new ECKey());
        saveAccount(account);
        return account.getAddress();
    }

    public Address addAccount(String passphrase) {
        Account account = new Account(new ECKey());
        saveAccount(account, passphrase);
        return account.getAddress();
    }

    public Address addAccount(Account account) {
        saveAccount(account);
        return account.getAddress();
    }
    ...
    public Account getAccount(Address addr, String passphrase) {
        synchronized (accessLock) {
    ...
    public boolean unlockAccount(Address addr, String passphrase, long duration) {
    ...
    public boolean unlockAccount(Address addr, String passphrase) {
        Account account;
    ...
    public boolean lockAccount(Address addr) {
        synchronized (accessLock) {
            if (!accounts.containsKey(addr)) {
                return false;
            }
    ...
    public byte[] addAccountWithPrivateKey(byte[] privateKeyBytes) {
        ...
        synchronized (accessLock) {
            Address addr = addAccount(account);
